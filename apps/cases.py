...
fig2 = daily_plots("Reported_Cases","moving_average_cases") #plot of daily reported infections
...
cumilative_cases_figure = cumulative_cases("Cum_Cases")
 ...
 #county names, covid-cases, death cases and recoveries
...
countrywide = html.Div([
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                        ...
                        html.H6(f"Female-{total_female_cases}({perc_female_cases:.2f}%)",  className = "text-end",style = {"color":"#998ec3"}),
                        html.H6(f"Male-{total_male_cases}({perc_male_cases:.2f}%)",className = "text-end",style = {"color":"#f1a340"}),
...
alert = dbc.Alert("Please choose County from dropdown in order to visualize the results!", color="danger",
                  dismissable=True),

countywide_cases = html.Div([
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H6("Filter by county", className = "text-dark"),
                    dcc.Dropdown(
                        id = "county_chosen" , 
...
def update_graph_card(county, start_date, end_date):
    if len(county) == 0:
        ...
        fig2 = px.line(data, x="date_of_lab_confirmation" , y = data["lab_results"].cumsum() , color = "county",color_discrete_map =colors_pellete)
        fig2.update_yaxes(title = "Cumulative Cases", showline=True, linewidth = 0.2, linecolor = "gray", gridcolor = "gainsboro")
        ...
        fig = px.line(data_average, x="date_of_lab_confirmation" , y = "moving_average" , color = "county",color_discrete_map =colors_pellete)
        fig.update_yaxes(title = "7-day average cases", showline=True, linewidth = 0.2, linecolor = "black", gridcolor = "gainsboro")
        ...
        fig.update_layout(hovermode="x unified",paper_bgcolor=pcolor,plot_bgcolor = pcolor,uniformtext_minsize = 3, font_color = "#4F4F4F", bargap =0.05)     
        
        return fig, fig2

#image_card_cases, graph_card_2])